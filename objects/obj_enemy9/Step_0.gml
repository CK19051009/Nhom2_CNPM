...
switch state {
	...
	case 3:  //Attack
		...
		if count_attack == 0 and (x >= _x and x<=_x + camera_get_view_width(view_camera[0]))and (y>= _y and y <= _y + camera_get_view_height(view_camera[0])){
				instance_create_layer(obj_player.x,obj_player.y-30,"Instances_up",bullet)
				count_attack = 1
				state = 1}
		hurt()
	break
